...
class ObjcMethod():
    ...
    def hook(self):
        # print header
        if use_rlog:
            print(f'{self.line[:-2]} {"{"}RLog(@">>>> - {self.full_name}"); ', end="")
        else:
        ...
        if self.ret_type == "void":
            if use_rlog:
                print('%%orig;RLog(@"<<<< - %s"); }' %(self.full_name))
            else:
                print('%%orig;[LogTool logDataFromNSString:@"<<<< - %s"]; }' %(self.full_name))

        else:
            if use_rlog:
                print('%s ret = %%orig;RLog([NSString stringWithFormat: @"<<<< - %s ==> ret value: %s", ret]);return ret; }' %(self.ret_type, self.full_name, map_format_specifier(self.ret_type)))
            else:
...
def usage():
    print("usage: python hookify.py [d:p:r:f:R:] [dir= pattern= regex= file=]")
    ...
    print("\nto Use RLog insted of Logtool:")
    print("\tpython hookify.py -R")

def main(argv):
    try:
        opts, args = getopt.getopt(argv[1:], 'd:p:r:f:R,', ['dir=', 'pattern=', 'regex=', 'file='])
